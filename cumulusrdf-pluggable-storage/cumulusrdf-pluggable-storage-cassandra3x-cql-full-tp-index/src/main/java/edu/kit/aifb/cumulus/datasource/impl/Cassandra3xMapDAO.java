 ...
 */
public class Cassandra3xMapDAO<K, V> implements MapDAO<K, V> {
	...
	Cassandra3xMapDAO(
		...
		_delegate = _isBidirectional
				? new Cassandra3xBidirectionalMapDAO<K, V>(_factory.getSession(), name, ttl, keySerializer, valueSerializer)
				: new Cassandra3xDefaultMapDAO<K, V>(_factory.getSession(), name, _isBidirectional, ttl, keySerializer, valueSerializer);
	}
